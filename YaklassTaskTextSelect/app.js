...
chrome.storage.sync.get({ 'appActive': true }, (value) => {
    if (!value.appActive) return;

    const taskhtml = document.getElementById('taskhtml');
    if (taskhtml) {
        taskhtml.style.userSelect = 'text';
        taskhtml.querySelectorAll('*').forEach((element) => element.oncopy = () => navigator.clipboard.writeText(window.getSelection()));
    }
});
